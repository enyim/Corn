...
function dobuild {
    ...
    Exec {
	$sln = gci *.sln | select -first 1
        dotnet $( ?? $pack ("pack", "--no-build", "--output", $OutputPath) "build" ) $sln $(Get-CommonMSBuildArgs) /p:version=$FullVersion /p:ContinuousIntegrationBuild=true /p:STANDARD_CI_SOURCE_REVISION_ID=$LongHash
    }
}
